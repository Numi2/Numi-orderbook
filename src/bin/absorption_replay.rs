...
use orderbook::insights::{
    validate_absorption_replay, AbsorptionConfig, AbsorptionReplayValidation,
};
use std::fs::File;
...
fn main() -> anyhow::Result<()> {
    ...
    let validation = validate_absorption_replay(&frames, args.absorption);
...
fn usage() {
    eprintln!(
        "usage: absorption_replay recorded_frames.bin [threshold options]\n\
...
fn validation_passed(validation: &AbsorptionReplayValidation) -> bool {
