...
fn main() -> anyhow::Result<()> {
    ...
    let report_ms: u64 = if args.len() > 6 {
    ...
    while off + 16 <= data.len() {
        let (incl_len, _) = if le {
            read_le_u32(&data, off + 8)
        } else {
            read_be_u32(&data, off + 8)
        };
        off += 16;
...
#[inline]
fn read_le_u32(b: &[u8], off: usize) -> (u32, usize) {
    (
...
#[inline]
fn read_be_u32(b: &[u8], off: usize) -> (u32, usize) {
    (
