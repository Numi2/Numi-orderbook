...
use crate::codec_raw::{OboAddV1, OboModifyV1, OboCancelV1, OboExecuteV1};
...
pub fn map_event_to_obo_parts(ev: &Event) -> (Option<u32>, Option<OboEventV1>) {
    match *ev {
        Event::Add { order_id, instr, px, qty, side } => {
            (Some(instr), Some(OboEventV1::Add(OboAddV1 {
                order_id,
        ...
        Event::Trade { instr, px, qty, maker_order_id, taker_side } => {
            if let Some(maker) = maker_order_id {
                ...
                (Some(instr), Some(OboEventV1::Execute(OboExecuteV1 {
